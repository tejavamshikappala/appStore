...
class AppStore extends Component {
  ...
  onChangingElement = event => {
    this.setState({SearchInput: event.target.value})
  }

  gettingResults = () => {
    const {ActiveTabId} = this.state
    const ForResults = appsList.filter(each => each.category === ActiveTabId)
    return ForResults
  }

  gettingSearchResults = forFilteredResults => {
    const {SearchInput} = this.state

    const ForResults = forFilteredResults.filter(every =>
      every.appName.toLowerCase().includes(SearchInput.toLowerCase()),
    )
    return ForResults
  }
  ...
  render() {
    const {ActiveTabId} = this.state
    const forFilteredResults = this.gettingResults()

    const forsearchRes = this.gettingSearchResults(forFilteredResults)

    return (
      <div className="forOuter">
        <div>
          <h1 className="forHeader">App Store</h1>
          <div className="forDiv">
            <input
              type="search"
              className="forSearch"
              placeholder="search"
              onChange={this.onChangingElement}
            />
            <img
        ...
        <ul className="forUl">
          {tabsList.map(each => (
            <TabItem
              ...
              onChangingFun={this.onChangingFun}
            />
          ))}
        </ul>
        <ul className="forUlListContainer">
          {forsearchRes.map(eachItem => (
            <AppItem eachItemDetails={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
